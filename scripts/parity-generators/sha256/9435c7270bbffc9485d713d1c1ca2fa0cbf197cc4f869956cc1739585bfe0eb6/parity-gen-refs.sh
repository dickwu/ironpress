...
if not isinstance(lock, dict) or lock.get("schema") != 5:
    errors.append("refs.lock must use PDF-oracle schema 5")
...
try:
    ...
    valid = (lock.get("schema") == 5 and isinstance(provenance, dict)
             and provenance
             and all(
...
render_one() {
  ...
  local html rel category id oracle oracle_current oracle_pdf pdf profile validation_prefix pages first_page sd
...
export -f render_one
export CHROMIUM CHROMIUM_ABS CASES ORACLES TMP FORCE PARITY PAGEDJS PAGEDJS_BIN PAGE_CSS VALIDATION_DPI FIXTURE_ID
...
print(f"wrote refs.lock PDF schema 5 ({len(fixtures)} fixture identities)")
PY
}
